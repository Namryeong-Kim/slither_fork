...
from join.utils.formatters.exceptions import FormatImpossible
from join.utils.formatters.utils.patches import apply_patch, create_diff
from join.utils.colors import green, yellow, red
from join.core.declarations.enum.comparable_enum import ComparableEnum
from join.output.output import Output, SupportedOutput
...
class AbstractDetector(metaclass=abc.ABCMeta):
    ARGUMENT = ""  # run the detector with join.py --ARGUMENT
    ...
    def __init__(
        self, compilation_unit: JoinCompilationUnit, join: "Join", logger: Logger
    ) -> None:
        ...
        if results and self.join.triage_mode:
            while True:
                ...
                try:
                    ...
                    self.join.save_results_to_hide(
    ...
    def generate_result(
        ...
        output = Output(
            ...
            markdown_root=self.join.markdown_root,
        )
    ...
    @staticmethod
    def _format(_compilation_unit: JoinCompilationUnit, _result: Dict) -> None:
        """Implement format"""
        return

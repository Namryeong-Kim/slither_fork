...
from join.core.declarations.custom_error.custom_error_contract import CustomErrorContract
from join.core.declarations.custom_error.custom_error_top_level import CustomErrorTopLevel
...
from join.core.declarations.solidity_type.type_alias import TypeAlias, TypeAliasTopLevel, TypeAliasContract
from join.core.declarations.solidity_type.array_type import ArrayType
from join.core.declarations.solidity_type.element_type import ElementaryType,ElementaryTypeName
from join.core.declarations.function.function_type import FunctionType
...
from join.core.declarations.function.function_type_variable import FunctionTypeVariable
from join.exceptions import JoinError
from join.solc_parsing.exceptions import ParsingError
from join.solc_parsing.expressions.expression_parsing import CallerContextExpression
...
def _add_type_references(type_found: Type, src: str, sl: "JoinCompilationUnit") -> None:
...
def parse_type(
    ...
    caller_context: Union[CallerContextExpression, "JoinCompilationUnitSolc"],
) -> Type:
    """
    caller_context can be a JoinCompilationUnitSolc because we recursively call the function
    and go up in the context's scope. If we are really lost we just go over the JoinCompilationUnitSolc
    ...
    from join.solc_parsing.expressions.expression_parsing import parse_expression
    from join.solc_parsing.variables.function_type_variable import FunctionTypeVariableSolc
    from join.solc_parsing.declarations.contract import ContractSolc
    from join.solc_parsing.declarations.function import FunctionSolc
    from join.solc_parsing.declarations.using_for_top_level import UsingForTopLevelSolc
    from join.solc_parsing.declarations.custom_error import CustomErrorSolc
    from join.solc_parsing.declarations.structure_top_level import StructureTopLevelSolc
    from join.solc_parsing.join_compilation_unit_solc import JoinCompilationUnitSolc
    from join.solc_parsing.variables.top_level_variable import TopLevelVariableSolc

    sl: "JoinCompilationUnit"
    ...
    if isinstance(caller_context, JoinCompilationUnitSolc) or (
        isinstance(caller_context, FunctionSolc) and caller_context.contract_parser is None
    ):
        structures_direct_access: List["Structure"]
        if isinstance(caller_context, JoinCompilationUnitSolc):
            ...
            next_context = caller_context.join_parser
    ...
    if t[key] == "IdentifierPath":
        ...
        raise JoinError("Solidity 0.8 not supported with the legacy AST")

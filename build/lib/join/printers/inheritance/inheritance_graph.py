...
from join.utils.inheritance_analysis import (
...
class PrinterInheritanceGraph(AbstractPrinter):
    ...
    def __init__(self, join, logger):
        super().__init__(join, logger)

        inheritance = [x.inheritance for x in join.contracts]
        self.inheritance = {item for sublist in inheritance for item in sublist}

        self.overshadowing_state_variables = {}
        shadows = detect_state_variable_shadowing(join.contracts)

...
from join.slithir.operations import (
    Send,
    Transfer,
    OperationWithLValue,
    HighLevelCall,
...
from join.core.declarations.function.modifier import Modifier
...
from join.utils.formatters.exceptions import FormatError, FormatImpossible
from join.utils.formatters.utils.patches import create_patch
from join.core.declarations.solidity_type.element_type import ElementaryTypeName

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("Join.Format")
...
###################################################################################
###################################################################################
def custom_format(compilation_unit: JoinCompilationUnit, result: Dict) -> None:
...
def _name_already_use(join: JoinCompilationUnit, name: str) -> bool:
    # Do not convert to a name used somewhere else
    if not KEY in join.context:
        all_names: Set[str] = set()
        for contract in join.contracts_derived:
        ...
        join.context[KEY] = all_names
    return name in join.context[KEY]


def _convert_CapWords(original_name: str, join: JoinCompilationUnit) -> str:
    ...
    if _name_already_use(join, name):
...
def _convert_mixedCase(
    original_name: Union[str, bytes], compilation_unit: JoinCompilationUnit
) -> str:
...
def _convert_UPPER_CASE_WITH_UNDERSCORES(
    name: str, compilation_unit: JoinCompilationUnit
) -> str:
...
CONVENTION_F_TYPE = Callable[[str, JoinCompilationUnit], str]
...
def _get_from_contract(
    compilation_unit: JoinCompilationUnit, element: Dict, name: str, getter: str
) -> TARGET_TYPE:
...
def _patch(
    compilation_unit: JoinCompilationUnit, result: Dict, element: Dict, _target: str
) -> None:
    scope = compilation_unit.get_scope(element["source_mapping"]["filename_absolute"])
...
def _is_var_declaration(join: JoinCompilationUnit, filename: str, start: int) -> bool:
    """
...
def _explore_type(  # pylint: disable=too-many-arguments,too-many-locals,too-many-branches
    ...
    if isinstance(custom_type, UserDefinedType):
        ...
        if isinstance(custom_type.type, (Enum, Contract)):
            if custom_type.type == target:
                ...
                new_str = convert(old_str, join)

                loc_start = start
                if _is_var_declaration(join, filename_source_code, start):
            ...
            _explore_variables_declaration(
                join, list(custom_type.type.elems.values()), result, target, convert
            )
    ...
    if isinstance(custom_type, MappingType):
        ...
        if isinstance(custom_type.type_to, (UserDefinedType, MappingType)) or target in [
            ...
            full_txt_end = end
            full_txt = join.core.source_code[filename_source_code].encode("utf8")[
            ...
            if patch_comment and isinstance(variable, LocalVariable):
                if variable.source_mapping.lines:
                    ...
                    if variable in func.parameters:
                        ...
                        potential_comments_ = join.core.source_code[filename_source_code].encode(
                            "utf8"
                        )
...
def _explore_structures_declaration(
    ...
    for st in structures:
        ...
        _explore_variables_declaration(join, list(st.elems.values()), result, target, convert)
...
def _explore_events_declaration(
    ...
    for event in events:
        ...
        _explore_variables_declaration(join, event.elems, result, target, convert)
...
def _explore_functions(
    ...
    for function in functions:
        _explore_variables_declaration(join, function.variables, result, target, convert, True)
        _explore_irs(join, function.all_slithir_operations(), result, target, convert)
...
def _explore_contract(
    ...
    _explore_variables_declaration(join, contract.state_variables, result, target, convert)
    _explore_structures_declaration(join, contract.structures, result, target, convert)
    _explore_functions(join, contract.functions_and_modifiers, result, target, convert)
    _explore_enums(join, contract.enums, result, target, convert)
...
def _explore(
    compilation_unit: JoinCompilationUnit,
    result: Dict,
    target: TARGET_TYPE,
    convert: CONVENTION_F_TYPE,
) -> None:

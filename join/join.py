...
from join.detectors.abstract_detector import AbstractDetector, DetectorClassification
...
from join.output.output import Output
...
class Join(JoinCore):  # pylint: disable=too-many-instance-attributes
    def __init__(self, target: Union[str, CryticCompile], **kwargs) -> None:
        ...
        self._parsers: List[JoinCompilationUnitSolc] = []
        try:
            ...
            raise JoinError(f"Invalid compilation: \n{str(e)}")
        for compilation_unit in crytic_compile.compilation_units.values():
            compilation_unit_join = JoinCompilationUnit(self, compilation_unit)
            self._compilation_units.append(compilation_unit_join)
            parser = JoinCompilationUnitSolc(compilation_unit_join)
            self._parsers.append(parser)
            for path, ast in compilation_unit.asts.items():
                parser.parse_top_level_from_loaded_json(ast, path)
                self.add_source_code(path)

            _update_file_scopes(compilation_unit_join.scopes.values())

...
from join.core.declarations.variables.solidity_variables import SolidityFunction
...
from join.printers.abstract_printer import AbstractPrinter
...
from join.utils.code_complexity import compute_cyclomatic_complexity
from join.utils.colors import green, red, yellow
from join.utils.myprettytable import MyPrettyTable
from join.utils.standard_libraries import is_standard_library
from join.core.cfg.node import NodeType
from join.utils.test_pattern import is_test_file
...
class PrinterHumanSummary(AbstractPrinter):
    ...
    def _get_detectors_result(self) -> Tuple[List[Dict], int, int, int, int, int]:
        ...
        checks_optimization = self.join.detectors_optimization
        checks_informational = self.join.detectors_informational
        checks_low = self.join.detectors_low
        checks_medium = self.join.detectors_medium
        checks_high = self.join.detectors_high
    ...
    def _lines_number(self):
        if not self.join.source_code:
            return None
        total_dep_lines = 0
        total_lines = 0
        total_tests_lines = 0

        for filename, source_code in self.join.source_code.items():
            lines = len(source_code.splitlines())
            is_dep = False
            if self.join.crytic_compile:
                is_dep = self.join.crytic_compile.is_dependency(filename)
            if is_dep:
    ...
    def _compilation_type(self):
        if self.join.crytic_compile is None:
            return "Compilation non standard\n"
        return f"Compiled with {str(self.join.crytic_compile.type)}\n"

    def _number_contracts(self):
        if self.join.crytic_compile is None:
            return len(self.join.contracts), 0, 0
        contracts = self.join.contracts
    ...
    def _get_features(self, contract):  # pylint: disable=too-many-branches
        ...
        for compilation_unit in self.join.compilation_units:
    ...
    def output(self, _filename):  # pylint: disable=too-many-locals,too-many-statements
        ...
        for contract in self.join.contracts_derived:

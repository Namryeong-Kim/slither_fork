...
from join.core.analyses.data_dependency.data_dependency import compute_dependency
...
from join.core.declarations.top_levels.enum_top_level import EnumTopLevel
from join.core.declarations.top_levels.function_top_level import FunctionTopLevel
...
from join.core.declarations.top_levels.structure_top_level import StructureTopLevel
from join.core.declarations.top_levels.using_for_top_level import UsingForTopLevel
...
from join.solc_parsing.exceptions import VariableNotFound
from join.solc_parsing.variables.top_level_variable import TopLevelVariableSolc

logging.basicConfig()
logger = logging.getLogger("JoinSolcParsing")
...
class JoinCompilationUnitSolc(CallerContextExpression):
    # pylint: disable=no-self-use,too-many-instance-attributes
    def __init__(self, compilation_unit: JoinCompilationUnit) -> None:
        super().__init__()
    ...
    def parse_top_level_from_loaded_json(self, data_loaded: Dict, filename: str) -> None:
        ...
        for top_level_data in data_loaded[self.get_children()]:
            if top_level_data[self.get_key()] == "ContractDefinition":
                ...
                raise JoinException(f"Top level {top_level_data[self.get_key()]} not supported")
    ...
    def parse_contracts(self) -> None:  # pylint: disable=too-many-statements,too-many-branches
        ...
        for contract in self._underlying_contract_to_parser:
            if contract.name.startswith("JoinInternalTopLevelContract"):
                raise JoinException(
                    # region multi-line-string
                    """Your codebase has a contract named 'JoinInternalTopLevelContract'.
Please rename it, this name is reserved for Join's internals"""
    ...
    def analyze_contracts(self) -> None:  # pylint: disable=too-many-statements,too-many-branches
        if not self._parsed:
            raise JoinException("Parse the contract before running analyses")
    ...
    def _analyze_top_level_structures(self) -> None:
        try:
            ...
            raise JoinException(f"Missing struct {e} during top level structure analyze") from e
    ...
    def _analyze_top_level_variables(self) -> None:
        try:
            ...
            raise JoinException(f"Missing {e} during variable analyze") from e
    ...
    def _analyze_content_top_level_function(self) -> None:
        try:
            ...
            raise JoinException(f"Missing {e} during top level function analyze") from e

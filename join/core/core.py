...
from join.core.declarations.contract.contract import Contract, FunctionContract
from join.core.declarations.top_levels.using_for_top_level import TopLevel
from join.core.source_mapping.source_mapping import SourceMapping, Source
from join.slithir.variables import Constant
from join.utils.colors import red
from join.core.source_mapping.source_mapping_utils import get_definition, get_references, get_implementation
...
class JoinCore(Context):
    """
    Join static analyzer
    """
    ...
    def __init__(self) -> None:
        ...
        self._previous_results_filename: str = "Join.db.json"
        self._results_to_hide: List = []
        self._previous_results: List = []
        # From triaged result
        self._previous_results_ids: Set[str] = set()
        # Every join object has a list of result from detector
        ...
        # If set to true, join will not catch errors during parsing
        ...
        self._compilation_units: List[JoinCompilationUnit] = []
    ...
    @property
    def compilation_units(self) -> List[JoinCompilationUnit]:
        return list(self._compilation_units)

    def add_compilation_unit(self, compilation_unit: JoinCompilationUnit):
    ...
    def parse_ignore_comments(self, file: str) -> None:
        ...
        while True:
            ...
            start_regex = r"^\s*//\s*join-disable-start\s*([a-zA-Z0-9_,-]*)"
            end_regex = r"^\s*//\s*join-disable-end\s*([a-zA-Z0-9_,-]*)"
                ...
                if ignored:
                    for check in ignored:
                        ...
                        if len(vals) == 0 or vals[-1][1] != float("inf"):
                            ...
                            logger.error(
                                f"Consecutive join-disable-starts without join-disable-end in {file}#{line_number}"
                            )
                            return
            ...
            if end_match:
                ...
                if ignored:
                    for check in ignored:
                        ...
                        if len(vals) == 0 or vals[-1][1] != float("inf"):
                            logger.error(
                                f"join-disable-end without join-disable-start in {file}#{line_number}"
                            )
                            return
    ...
    def has_ignore_comment(self, r: Dict) -> bool:
        ...
        for file, lines in mapping_elements_with_lines:
            ...
            if ignore_line_text:
                match = re.findall(
                    r"^\s*//\s*join-disable-next-line\s*([a-zA-Z0-9_,-]*)",
        ...
        for path in self._paths_to_filter:
            try:
                ...
                logger.error(
                    ...
                    "\nJoin supports the Python re format"
